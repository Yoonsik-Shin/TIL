...
```sql
SELECT FirstName AS "성", LastName AS "이름" 
...
```sql 
SELECT count(*) AS "고객 수", Country, State FROM customers WHERE State IS NOT NULL AND State <> '' GROUP BY Country, State ORDER BY "고객수" DESC, Country DESC LIMIT 5;
```
...
```sql 
SELECT LastName, FirstName, cast(strftime('%Y', datetime('now')) as int) - cast(strftime('%Y', BirthDate) as int)  + 1 AS "나이" FROM employees ORDER BY EmployeeId ASC;
```
...
SELECT Name 
FROM artists 
WHERE ArtistId = (SELECT ArtistId FROM (SELECT ArtistId, max(a) FROM (SELECT ArtistId, count(*) AS a FROM albums GROUP BY ArtistId)));
```
...
SELECT Name
FROM genres
WHERE GenreId = (SELECT GenreId FROM (SELECT GenreId, min(a) FROM (SELECT GenreId, count(*) AS a FROM tracks GROUP BY GenreId)));
```
