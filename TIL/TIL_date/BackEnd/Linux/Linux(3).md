...
- 첫번째 명령어의 표준출력을 두번째 명령어의 표준 입력으로 넣어줄 수 있음
...
- 표준출력을 복사해서 파일로 내보내고, 다음 명령어에 표준입력으로 넘겨줌
...
> [-] 와 다른점 : 중괄호는 단순히 안에 맞는 문자열을 생성하여 출력
...
```bash
$ echo "hello       hi{1,2,3} $PATH"
>> hello       hi{1,2,3} 변수값출력
```
