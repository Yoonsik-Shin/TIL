...
```python
action = Genre.objects.get(title='드라마')
...
```python
lastest_movie = Movie.objects.order_by('-opening_date')

for i in lastest_movie:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
oldest_movie = Movie.objects.order_by('opening_date')[0]

print(oldest_movie.director.name, oldest_movie.genre.title, oldest_movie.title, oldest_movie.opening_date, oldest_movie.running_time, i.screening)
```
...
```python
screening = Movie.objects.filter(screening='True').order_by('opening_date')
...
```python
director_ = Director.objects.get(name='봉준호')
bong_movies = Movie.objects.filter(director = director_).order_by('opening_date')

for i in bong_movies:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
director_ = Director.objects.get(name='봉준호')
bong_movies = Movie.objects.filter(director = director_).order_by('opening_date')[1]
i = bong_movies
...
```python
long_movie = Movie.objects.filter(running_time__gt=119).order_by('running_time')
...
```python
long_movie = Movie.objects.filter(running_time__gte=119).order_by('running_time')
...
before_movie = Movie.objects.filter(opening_date__gte=datetime.date(2022,1,1)).order_by('opening_date')

for i in before_movie:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
bong_drama_movie = Movie.objects.filter(director=bong, genre=drama).order_by('opening_date')

for i in bong_drama_movie:
    print(i.director.name, i.genre.title, i.title, i.opening_date, i.running_time, i.screening)
```
...
not_bong = Movie.objects.exclude(director=bong).order_by('opening_date')
